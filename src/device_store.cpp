...
size_t DeviceStore::buildUplinkBatch(String& outDevicesJson,
  ...
  for (auto& kv : devices_) {
    ...
    // "Dirty" = seen again (or newly) since the last successful uplink.
    if (d.lastSeen <= d.uplinkedAt && d.uplinkedAt != 0) continue;
    if (n) outDevicesJson += ",";
    appendDeviceJson(outDevicesJson, d);
...
void DeviceStore::markUplinked(const std::vector<uint64_t>& keys) {
  ...
  for (uint64_t k : keys) {
    ...
    if (it != devices_.end()) it->second.uplinkedAt = it->second.lastSeen;
  }
  unlock();
}
...
size_t DeviceStore::pendingUplink() {
  ...
  for (auto& kv : devices_) {
    const BleDevice& d = kv.second;
    if (d.lastSeen > d.uplinkedAt || d.uplinkedAt == 0) n++;
  }
  unlock();
  return n;
}
